window.onload = function () {
  ...
  // 인기상품
  let POPULAR_GOOD;
  let popularShow = 1; //목록중에 0번을 보여줌.
  ...
  function showPopularIconGood() {
    ...
    tag.forEach(function (item, index) {
      ...
      // 클릭 하면 버튼(.popular-more)의 글자를
      // 클릭된 타이틀의 글자로 변경한다.
      item.addEventListener("click", function (event) {
        // a태그 이므로 href 적용함.
        // 웹브라우저 갱신이 되므로 ui를 위해 막아야 함.
      ...
      <li >
       <div class="season-good clearfix">
        ...
        <p class = "season-good-info">
          <a href="${item.link}" class="season-good-title">${item.title}
          </a>
          <a href="${item.link}" class="season-good-price">
            <em>${priceToString(item.price)}
            </em>원
          </a>
        </p>
       </div>
  ...
  // 커뮤니티 탭메뉴
  // 탭 버튼
  const tabBtArr = document.querySelectorAll(".coumunity-bt");
  // 탭 내용
  const tabConArr = document.querySelectorAll(".comunity-notice dd");
  // 탭 포커스
  let tabFocusIndex = 0;
  // 탭 버튼 클릭 처리
  ...
  function tabfocusFn() {
    ...
    tabBtArr.forEach(function (item) {
      item.classList.remove("comunity-bt-active");
    });
    // 인덱스에 해당하는 것만 적용
    tabBtArr[tabFocusIndex].classList.add("comunity-bt-active");
    // 내용에서 일단 모두 제거
    tabConArr.forEach(function (item) {
      item.classList.remove("comunity-visible-active");
    });
    tabConArr[tabFocusIndex].classList.add("comunity-visible-active");
  }
  // --------------------------------
  // 전체 체크박스 기능
